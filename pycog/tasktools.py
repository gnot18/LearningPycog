...
def performance_2afc(trials, z):                         # z: z.get_value (function in theano) return ndarray z containing outputs for all times, trials, unit action in the validation_minibatch
    ...
    choices = [np.argmax(z[ends[i],i,:2]) for i, end in enumerate(ends)]  # a list of an index of the max value of z for every times and trials (unit number) : size : trials
    for i, end in enumerate(ends):
        choice_idx = np.argmax(z[ends[i],i,:2])
        if z[ends[i],i,choice_idx] < 4.25:
            choices[i] = None
